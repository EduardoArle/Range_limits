...
#load models minT_relPol
setwd(wd_models)
models <- lapply(list.files(pattern = '^minT_.*relPol_GS$'), readRDS)
names(models) <- list.files(pattern = '^minT_.*relPol_GS$')

#load sigs
setwd(wd_sig)
sigs <- lapply(list.files(pattern = '^minT_.*relPol_GS\\.csv$'), read.csv)
names(sigs) <- gsub('\\.csv', '', list.files(pattern = '^minT_.*relPol_GS\\.csv$'))
...
#plot A.i (minT relative polewardness) RESTRICTED BY LATITUDE 60

plot_gam_shade_sig(models$minT_rest60_relPol_GS, select = 1, col = "#0000FF",
                   xlim = c(0, 1), ylim = c(-0.3, 0.3),
                   sig_x = sigs$minT_rest60_relPol_GS$relPolewardness,
                   sig = sigs$minT_rest60_relPol_GS$sig,
                   add_axes = TRUE, axis_pos = -0.3,
                   xlab = "Relative polewardness",
                   ylab = "SHAP value")

#look at stats
summary(models$minT_rest60_relPol_GS)

#save 800


#plot A.i (minT relative polewardness) RESTRICTED BY LATITUDE 40

plot_gam_shade_sig(models$minT_rest40_relPol_GS, select = 1, col = "#0000FF",
                   xlim = c(0, 1), ylim = c(-0.3, 0.3),
                   sig_x = sigs$minT_rest40_relPol_GS$relPolewardness,
                   sig = sigs$minT_rest40_relPol_GS$sig,
                   add_axes = TRUE, axis_pos = -0.3,
                   xlab = "Relative polewardness",
                   ylab = "SHAP value")

#look at stats
summary(models$minT_rest40_relPol_GS)

#save 800



#load models meanT_relPol
setwd(wd_models)
models <- lapply(list.files(pattern = '^meanT_.*relPol_GS$'), readRDS)
names(models) <- list.files(pattern = '^meanT_.*relPol_GS$')

#load sigs
setwd(wd_sig)
sigs <- lapply(list.files(pattern = '^meanT_.*relPol_GS\\.csv$'), read.csv)
names(sigs) <- gsub('\\.csv', '', list.files(pattern = '^meanT_.*relPol_GS\\.csv$'))
...
#plot A.ii (meanT relative polewardness) RESTRICTED BY LATITUDE 60

plot_gam_shade_sig(models$meanT_rest60_relPol_GS, select = 1, col = "#800080",
                   xlim = c(0, 1), ylim = c(-0.3, 0.3),
                   sig_x = sigs$meanT_rest60_relPol_GS$relPolewardness,
                   sig = sigs$meanT_rest60_relPol_GS$sig,
                   add_axes = TRUE, axis_pos = -0.3,
                   xlab = "Relative polewardness",
                   ylab = "")

#look at stats
summary(models$meanT_rest60_relPol_GS)

#save 800


#plot A.ii (meanT relative polewardness) RESTRICTED BY LATITUDE 40

plot_gam_shade_sig(models$meanT_rest40_relPol_GS, select = 1, col = "#800080",
                   xlim = c(0, 1), ylim = c(-0.3, 0.3),
                   sig_x = sigs$meanT_rest40_relPol_GS$relPolewardness,
                   sig = sigs$meanT_rest40_relPol_GS$sig,
                   add_axes = TRUE, axis_pos = -0.3,
                   xlab = "Relative polewardness",
                   ylab = "")

#look at stats
summary(models$meanT_rest40_relPol_GS)

#save 800


#load models maxT_relPol
setwd(wd_models)
models <- lapply(list.files(pattern = '^maxT_.*relPol_GS$'), readRDS)
names(models) <- list.files(pattern = '^maxT_.*relPol_GS$')

#load sigs
setwd(wd_sig)
sigs <- lapply(list.files(pattern = '^maxT_.*relPol_GS\\.csv$'), read.csv)
names(sigs) <- gsub('\\.csv', '', list.files(pattern = '^maxT_.*relPol_GS\\.csv$'))
...
#plot A.iii (maxT relative polewardness) RESTRICTED BY LATITUDE 60

plot_gam_shade_sig(models$maxT_rest60_relPol_GS, select = 1, col = "#FF0000",
                   xlim = c(0, 1), ylim = c(-0.3, 0.3),
                   sig_x = sigs$maxT_rest60_relPol_GS$relPolewardness,
                   sig = sigs$maxT_rest60_relPol_GS$sig,
                   add_axes = TRUE, axis_pos = -0.3,
                   xlab = "Relative polewardness",
                   ylab = "")

#look at stats
summary(models$maxT_rest60_relPol_GS)

#save 800


#plot A.iii (maxT relative polewardness) RESTRICTED BY LATITUDE 40

plot_gam_shade_sig(models$maxT_rest40_relPol_GS, select = 1, col = "#FF0000",
                   xlim = c(0, 1), ylim = c(-0.3, 0.3),
                   sig_x = sigs$maxT_rest40_relPol_GS$relPolewardness,
                   sig = sigs$maxT_rest40_relPol_GS$sig,
                   add_axes = TRUE, axis_pos = -0.3,
                   xlab = "Relative polewardness",
                   ylab = "")

#look at stats
summary(models$maxT_rest40_relPol_GS)
