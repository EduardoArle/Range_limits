...
# Generate 33, 34, and 33 random values for distEdgeNormal
distEdge_A <- runif(33, min = 0, max = 83) #1st segment
distEdge_B <- runif(34, min = 84, max = 166) #2nd segment
distEdge_C <- runif(33, min = 167, max = 250) #2nd segment

# Generate ydata with a non-linear relationship plus some noise
ydata_A <-
  ((1.8 * distEdge_A + rnorm(33, mean = 0.4, sd = 0.1)) - 0.25) 

ydata_B <-
  ((distEdge_B + rnorm(34, mean = 0.2, sd = 0.3)) / 6) 

ydata_C <-
  ((1.5 * distEdge_C + rnorm(33, mean = 0, sd = 0.1)) - 1.2) 


#combine into a data frame
data <- data.frame(distEdge = c(relPol_A, relPol_B, relPol_C),
                   ydata = c(ydata_A, ydata_B, ydata_C))
